...
streamlit run visual_eureka/app.py
```

On macOS, prefix with `caffeinate -i` to prevent idle sleep during long overnight runs.
...
### Go2 Quadruped — Manual Baseline (v4, best known)

Trained manually to validate the environment and reward design principles before running the automated loop. Used stability-gated velocity reward: `forward_vel = 10.0 * qvel[0] * exp(-8 * (roll² + pitch²))`.

| Metric | Value |
|---|---|
| Training steps | 2M |
| Forward velocity | 0.97 m/s |
| Episode length | 2000 steps (full) |
| Success rate | 100% |

![Go2 walking — v4 manual baseline](results/go2_v4_manual_baseline.gif)

### Go2 Quadruped — LLM Loop Iteration 0 (first automated run)

First reward function generated by the automated Eureka loop (`nvidia/nemotron-3.5-lightning-30b-a3b`). Robot achieves forward motion but falls after ~112 steps — orientation penalty high, stability not yet learned.

| Metric | Value |
|---|---|
| Training steps | 500K (filter: 50K × 2 candidates) |
| Mean reward | 397.85 |
| Episode length | 111.7 steps |
| Forward velocity | 1.83/step |
| Success rate | 0% |

![Go2 LLM iteration 0 — rollout 1](results/llm_iter0_rollout1.gif) ![Go2 LLM iteration 0 — rollout 2](results/llm_iter0_rollout2.gif)
