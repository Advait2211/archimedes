"""Go2 v4: stability-gated velocity reward — only reward forward motion when upright."""
...
def compute_reward(obs, info, data, model):
    ...
    stability = np.exp(-8.0 * (roll**2 + pitch**2))

    forward_vel = data.qvel[0]
    forward_reward = 10.0 * forward_vel * stability
    ...
    height = data.qpos[2]
    height_reward = 2.0 * np.exp(-30.0 * (height - 0.27)**2)

    action = info["action"]
    action_pen = -0.02 * np.sum(action**2)

    ang_vel_pen = -0.8 * np.sum(data.qvel[3:6]**2)
...
def main():
    ...
    save_path = "state/models/go2_v4_2m.zip"
    print("Training 2M steps — stability-gated velocity reward...")
    t0 = time.time()
    ...
    gif_paths, thumb_path = render_eval_gifs(
        ...
        base_output_path="state/models/go2_v4_eval.gif",
        n_steps=500,
        n_rollouts=3,
    )
